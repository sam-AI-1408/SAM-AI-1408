...
def voice_command():
    ...
    try:
        ...
        if "hello" in cmd or "hi" in cmd:
            response_text = f"Hi {current_user.username}, how can I help you today?"
        elif "how are you" in cmd:
            response_text = "I'm doing great! Ready to help you with your tasks."
        ...
        # Task management
            ...
            response_text = "Please enter the task title in the dashboard to add it."
        elif "complete task" in cmd:
            response_text = "Please select the task in the dashboard to complete it."

        # Quests
        elif "complete quest" in cmd:
            response_text = "Please select the quest to mark it as completed."
        ...
        # Terminate
        elif "terminate" in cmd or "close assistant" in cmd:
            response_text = "Voice assistant closed."

    except Exception as e:
        response_text = f"Error processing command: {str(e)}"

    # Return text only (remove TTS for now if using offline female voice)
...
def speak():
    ...
    audio_url = "/static/audio/crush_voice.mp3"  # fallback

    try:
        url = "https://www.minimax.io/api/v1/audio"
        headers = {
        ...
        payload = {
            ...
            "group_id": "1963774935595750303",
            "format": "mp3"
        }

        res = requests.post(url, headers=headers, json=payload, timeout=15)
        if res.status_code == 200:
            audio_url = res.json().get("url", audio_url)
        else:
            print("Minimax error:", res.text)
